...
Mesh::Mesh(const float p_vert[][2], int p_nvert,const int p_tri[][3],int p_ntri) {
    for (int i=0; i<p_nvert; i++) {
        vertices.push_back(Vector2D(p_vert[i][0],p_vert[i][1]));
    }
    for (int i=0; i<p_ntri; i++) {
        triangles.push_back(Triangle(&vertices[p_tri[i][0]],
...
void Mesh::flip(const Triangle ptr1, const Triangle ptr2) {
    // search common edge
    Vector2D *P = ptr1.getVertexNotIn(ptr2);
    Vector2D *Q = ptr2.getVertexNotIn(ptr1);
    Vector2D *R = ptr1.getNextVertex(P);
    triangles.push_back(Triangle(P,R,Q));
    //ptr1->updateVertices(P,R,Q);
    R = ptr2.getNextVertex(Q);
    triangles.push_back(Triangle(Q,R,P));
    //ptr2->updateVertices(Q,R,P);
    auto it = triangles.begin();
    while (it!=triangles.end() && ((*it))!=ptr1) it++;
    if (it!=triangles.end()) {
        cout << "erase: " << it->whoami(vertices) << endl;
        triangles.erase(it);
    }
    it = triangles.begin();
    while (it!=triangles.end() && ((*it))!=ptr2) it++;
    if (it!=triangles.end()) {
        cout << "erase: " << it->whoami(vertices) << endl;
        triangles.erase(it);
    }
...
void Mesh::subdivide(const Vector2D &P) {
    ...
    if (it!=triangles.end()) {
        int i = vertices.size();
        cout << "indice=" << i << endl;
        vertices.push_back(Vector2D(P.x,P.y));
        Vector2D *v = &vertices[i];
        cout << P << "->" << *v << endl;

        triangles.push_back(Triangle(it->ptr[0], v, it->ptr[2]));
        triangles.push_back(Triangle(it->ptr[1], it->ptr[2], v));
        triangles.push_back(Triangle(it->ptr[0], it->ptr[1], v));
